...
class Bam2taTests {
    ...
    @AfterEach fun cleanup() = cleanupTest()
     ...
     @Test fun `run bam2ta step - se disable tn5 shift `() {
       ...
       cmdRunner.bam2ta(BAM,"chrM",true,"chrM",0,"output",false,false,1, testOutputDir)
       assertThat(testOutputDir.resolve("output.tagAlign.gz")).exists()
    }
    @Test fun `run bam2ta step - se control disable tn5 shift`() {

        cmdRunner.bam2ta(CONTROL,"chrM",true,"chrM",0,"output",false,false,1, testOutputDir)
        assertThat(testOutputDir.resolve("output.tagAlign.gz")).exists()
    }
    @Test fun `run bam2ta step - se disable tn5 shift subsample`() {

        cmdRunner.bam2ta(BAM,"chrM",true,"chrM",3,"output",false,false,1, testOutputDir)
        assertThat(testOutputDir.resolve("output.tagAlign.gz")).exists()
    ...
    @Test fun `run bam2ta step - se tn5 shift subsample`() {
        ...
        cmdRunner.bam2ta(BAM,"chrM",false,"chrM",2,"output",false,false,1, testOutputDir)
        assertThat(testOutputDir.resolve("output.tagAlign.gz")).exists()
     ...
     @Test fun `run bam2ta step - pe disable tn5 shift`() {
        ...
        cmdRunner.bam2ta(PBAM,"chrM",true,"chrM",0,"output",false,true,1, testOutputDir)
         assertThat(testOutputDir.resolve("output.tagAlign.gz")).exists()
     }
     @Test fun `run bam2ta step - pe  tn5 shift`() {

        cmdRunner.bam2ta(PBAM,"chrM",false,"chrM",0,"output",false,true,1, testOutputDir)
         assertThat(testOutputDir.resolve("output.tagAlign.gz")).exists()
         assertThat(testOutputDir.resolve("output.tn5.tagAlign.gz")).exists()
     }
    @Test fun `run bam2ta step - pe  tn5 shift subsample`() {

        cmdRunner.bam2ta(PBAM,"chrM",false,"chrM",2,"output",false,true,1, testOutputDir)
        assertThat(testOutputDir.resolve("output.2.tagAlign.gz")).exists()
        assertThat(testOutputDir.resolve("output.tagAlign.gz")).exists()
        assertThat(testOutputDir.resolve("output.tn5.tagAlign.gz")).exists()
    }
